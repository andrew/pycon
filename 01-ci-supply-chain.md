...
The largest package in the OIDC set is around 1.1M monthly downloads; it's in coordinated disclosure so unnamed here. Its workflow interpolates `github.event.comment.body` into a shell test with no author check and no `permissions:` block. The publish workflow accepts `workflow_dispatch` and does declare `environment:` on the publish job, so whether the pivot completes depends on whether that environment has protection rules configured, which we can't see from the YAML.
...
`bot-conditions` (`slice_bot_conditions.tsv`): 873 repos gate something on `github.actor == 'dependabot[bot]'` or similar, which zizmor flags as spoofable under certain re-run and event conditions. Several very-high-download packages share an `auto-merge.yml` with this check. I haven't verified what the auto-merge actually grants; if it's "merge Dependabot PRs without review" the question is whether an attacker can get their own PR into a state where the actor reads as `dependabot[bot]`. zizmor's docs say yes in narrow cases. Needs hands-on confirmation before naming anyone.
